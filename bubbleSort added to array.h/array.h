...
void bubbleSort(T array[], int N){
  ...
  int temp; //temporary variable to swap and put number in largest to smallest order
  while(done ==true){
    done = false;
    for(int i=1;i<N;i++){ //for loop to go through each number in array
      if(array[i]>array[i-1]){ //the bubble sort checking the value of the index next to initial index
	temp=array[i-1];
	array[i-1]=array[i];
	array[i]=temp;
	done=true; //setting back to true in order to continue through array to bubble sort
      }
    }
  }
}
